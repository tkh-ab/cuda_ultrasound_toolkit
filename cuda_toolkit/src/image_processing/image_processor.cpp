										...
										const NccMotionParameters& params)
{
	...
	for( uint i = 0; i < d_input_images.size(); ++i)
	{
		...
		result &= _compare_images2( *template_image, *source_image, d_motion_map + i * motion_map_count, image_dims, params);
...
bool
ImageProcessor::_compare_images2(const PitchedArray<float>& template_image,
						const PitchedArray<float>& source_image,
						int2* d_motion_map, 
						uint2 image_dims, 
						const NccMotionParameters& params)
